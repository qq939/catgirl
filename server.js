...
// 通用还原指令：强调神态、动作、构图、场景、细节一一对应
const RESTORE_INSTRUCTION = `严格1:1还原参考图的神态、动作、构图、场景，每一个细节都必须对应替换——头部朝向、眼神方向、嘴部状态、肢体位置、手/爪部动作、身体重心、与物品的交互方式、背景布局，全部原样映射到替换角色上，不允许遗漏或自由发挥。`;

// 病娇小少妇角色描写
const HUMAN_CHARACTER = `25岁少妇，苍白皮肤，黑微卷长发，病娇气质，眼神慵懒空洞又带一丝满足，白衬衫微敞领口，黑包臀裙，领口别萝莉胸针，脖子挂工牌绳（工牌证件照是穿正装的二次元萝莉动漫角色大头照，蓝底标准证件照排版），身边常见萝莉手办。`;

// 猫咪角色描写
const CAT_CHARACTER = `胖橘白相间猫咪，圆脸大眼，粉鼻头，白色胡须，毛色橘白交替，慵懒满足的神态，粉嫩肉垫。`;

function buildSinglePrompt(direction) {
  if (direction === 'cat-to-human') {
    return `将参考图中的猫咪替换为一个人物。${RESTORE_INSTRUCTION}替换后的人物为：${HUMAN_CHARACTER}构图与场景与参考图保持完全一致。写实摄影电影画质伦勃朗光。`;
  } else {
    return `将参考图中的人物替换为一只猫。${RESTORE_INSTRUCTION}替换后的猫咪为：${CAT_CHARACTER}构图与场景与参考图保持完全一致。写实摄影电影画质。`;
  }
}

function buildDualPrompt(direction) {
  if (direction === 'generate-human') {
    return `参考图为一只猫咪，请将猫咪替换为一个人物。${RESTORE_INSTRUCTION}替换后的人物为：${HUMAN_CHARACTER}构图与场景与参考图保持完全一致。写实摄影电影画质伦勃朗光。`;
  } else {
    return `参考图为一个人物，请将人物替换为一只猫。${RESTORE_INSTRUCTION}替换后的猫咪为：${CAT_CHARACTER}构图与场景与参考图保持完全一致。写实摄影电影画质。`;
  }
}
